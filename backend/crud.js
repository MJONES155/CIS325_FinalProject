...
app.post('/login', (req, res) => {
    ...
    db.get("SELECT * FROM User WHERE username = ?", [username], async (err, user) => {
        ...
        const token = jwt.sign({ userId: user.UserID }, secretKey, { expiresIn: '20m' });
...
app.post('/event', authenticateToken, (req, res) => {
    ...
    const useridevent = req.user.userId;

    db.run(
        "INSERT INTO Event (Title, Start, End, Category, UserID) VALUES (?, ?, ?, ?, ?)",
        [title, start, end, category, useridevent],
        function(err) {
            if (err) {
                return res.status(400).json({ message: err.message });
            }
...
app.put('/event/:EventID', authenticateToken, (req, res) => {
    ...
    db.run(query, [title, start, end, category, EventID], function (err) {
        ...
        res.json({ message: `Record with ID ${EventID} updated successfully` });
    });
});
...
app.post('/people', authenticateToken, (req, res) => {
    ...
    const useridpeople = req.user.userId;
    
    db.run(
        "INSERT INTO People (FirstName, LastName, Email, Category, UserID) VALUES (?, ?, ?, ?, ?)",
        [firstname, lastname, email, category, useridpeople], 
        function (err) {
            if (err) {
                return res.status(400).json({ message: err.message });
            }
...
//Update
app.put('/people/:PeopleID', authenticateToken, (req, res) => {
    ...
    db.run(query, [firstname, lastname, email, category, PeopleID], function (err) {
        ...
        res.json({ message: `Person with ID ${PeopleID} updated successfully` });
    });
});
...
//Delete
app.delete('/people/:PeopleID', authenticateToken,  (req, res) => {
    const { PeopleID } = req.params;
...
app.post('/work', authenticateToken, (req, res) => {
    ...
    const useridwork = req.user.userId;

    db.run(
        "INSERT INTO Work (Title, Start, End, UserID) VALUES (?, ?, ?, ?)",
        [title, start, end, useridwork],
        function(err) {
            if (err) {
                return res.status(400).json({ message: err.message });
            }
...
//Update
app.put('/work/:WorkID', authenticateToken, (req, res) => {
    ...
    db.run(query, [title, start, end, WorkID], function (err) {
        ...
        res.json({ message: `Record with ID ${WorkID} updated successfully`});
    });
});
  
//Delete
app.delete('/work/:WorkID', authenticateToken, (req, res) => {
    const { WorkID } = req.params;
...
app.post('/class', authenticateToken, (req, res) => {
    ...
    const useridclass = req.user.userId;
  
    db.run(
        "INSERT INTO Class (Title, Time, StartDate, EndDate, DaysOfTheWeek, Teacher, UserID) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [title, time, startdate, enddate, daysoftheweek, teacher, useridclass],
        function(err) {
            if (err) {
                return res.status(400).json({ message: err.message });
            }
...
app.post('/hobby', authenticateToken, (req, res) => {
    ...
    const useridhobby = req.user.userId;

    db.run(
        "INSERT INTO Hobby (HobbyName, Start, End, UserID) VALUES (?, ?, ?, ?)",
        [hobbyname, start, end, useridhobby],
        function(err) {
            if (err) {
...
//Update
app.put('/hobby/:HobbyID', authenticateToken, (req, res) => {
    ...
    db.run(query, [hobbyname, start, end, HobbyID], function (err) {
        ...
        res.json({ message: `Record with ID ${HobbyID} updated successfully` });
    });
});
  
//Delete
app.delete('/hobby/:HobbyID', authenticateToken, (req, res) => {
    const { HobbyID } = req.params;
